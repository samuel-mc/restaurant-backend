...
/**
 * Rate limit en memoria para el onboarding público ({@code POST /tenants/register}).
 * Suficiente en despliegue de una sola instancia; sin Redis.
 */
@Service
public class RegistrationRateLimitService {
    ...
    public RegistrationRateLimitService(
            @Value("${application.security.registration.max-attempts-per-ip:5}") int maxAttempts,
            @Value("${application.security.registration.window-minutes:60}") long windowMinutes
    ) {
        if (maxAttempts < 1) {
            throw new IllegalArgumentException(
                    "application.security.registration.max-attempts-per-ip debe ser >= 1."
            );
        }
        if (windowMinutes < 1) {
            throw new IllegalArgumentException(
                    "application.security.registration.window-minutes debe ser >= 1."
            );
        }
    ...
    /** Cuenta un intento (éxito o rechazo de negocio) dentro de la ventana. */
